...
   for the integrated rusty-startscript
...
Deutsch
---------
1. Das Programm ausführen. Beim erstmaligen Start wird ein Config-Ordner 
   mit den entsprechenden Dateien erstellt

2. Die Dateien "BPwipe.config" sowie "wipe.config" mit Notepad++ bearbeiten 
   und die Pfade der zu löschenden Dateien entsprechend angeben.

3. In der Datei "RUSTAiO.config" können allgemeine Programmeinstellungen
   vorgenommen werden.

4. In der Datei "rustide.config" befinden sich die Einstellungen
   für das integrierte rustide-startscript 
   (falls gewünscht und in den Einstellungen aktiviert)

5. Das Programm erneut starten und dem Programmablauf folgen


---> Falls das integrierte rustide-startscript nicht genutzt wird,
     kann in der Config ein separates Programm oder Script gewähl werden!

---> Die Hostname-Autoupdate Funktion fügt deinem angegebenen Hostname
     automatisch das Aktuelle Wipedatum hinzu und ändert ihn in deiner server.cfg!
     "YourHostname 13/10 WIPED"  In diesem Fall wird "dd/mm WIPED" automatisch eingefügt!

---> Die MapSize sowie der Seed können während des Wipevorgangs eingegeben werden und werden 
     automatisch in der server.cfg geändert
	 
---> Die Dateien "7za.exe" und "wget.exe" werden für das integrierte ServerSartScript benötigt
